...
import { Compass, Maximize2, Minimize2, Navigation, Map as MapIcon, X } from 'lucide-react';
...
export const Minimap: React.FC<MinimapProps> = ({
  ...
  return (
    <>
      {/* Mobile Floating Radar Button (Visible only on small screens when closed) */}
      <div className="fixed bottom-4 left-3 z-30 sm:hidden">
        {!isMobileOpen && (
          <button
      ...
      {/* Radar Map Container (Always visible on >=sm, toggleable on mobile) */}
      <div
        ...
        className={`fixed transition-all duration-300 z-30 ${
          ...
          isExpanded
            ? 'bottom-4 sm:bottom-6 left-3 sm:left-6 w-[calc(100vw-24px)] max-w-sm sm:max-w-none h-80 sm:w-[440px] sm:h-[440px]'
            : 'bottom-4 sm:bottom-6 left-3 sm:left-6 w-64 h-64 sm:w-60 sm:h-60'
        } rounded-2xl border border-stone-700/80 bg-stone-950/90 backdrop-blur-md shadow-2xl p-3 flex-col select-none`}
      >
        {/* Map Header */}
        ...
        <div
          ...
          className="relative flex-1 mt-2 rounded-xl bg-stone-900/90 border border-stone-800/80 overflow-hidden cursor-crosshair group select-none touch-none"
        >
          ...
          {exhibits.map((exhibit) => {
            ...
            return (
              <div
                ...
                className="absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer z-10 transition-transform duration-200 hover:scale-150 p-2 -m-2"
              >
                <div
